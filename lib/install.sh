...
if [ $(which npm) ]; then
  ...
  npm install -g eslint sitemap-generator-cli webpack svgo n gzip-size-cli flow-bin npx gatsby-cli create-react-native-app flow-typed create-react-app react-native-cli prettier git-open http-server depcheck
